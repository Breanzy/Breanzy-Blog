...
/** Corner L-bracket HUD with live time + coordinate readouts + animated bullshit meter. */
export default function HeroHUD({ compact = false }: HeroHUDProps) {
    ...
    // Slowly creep up to 100 — starts at a random offset, ticks every ~420ms
    useEffect(() => {
        if (bullshit >= 100) return;
        const id = setTimeout(
            () => setBullshit((v) => Math.min(100, v + Math.random() * 1.4 + 0.3)),
            420 + Math.random() * 180
        );
    ...
    const uptime = `00:${pad(Math.floor(seconds / 60))}:${pad(seconds % 60)}`;
    ...
    const Bracket = ({ which }: { which: string }) => (
    ...
    return (
        <div aria-hidden className="absolute inset-0 pointer-events-none z-[18]">
            ...
            {!compact && (
                <>
                    <div
                        className="absolute top-6 left-12 font-mono text-[10px] tracking-[0.2em] uppercase leading-relaxed"
                        style={{ color: "rgb(80 140 230 / 0.7)" }}
                    >
                        <div>sys.status <span className="text-emerald-400/80">● ONLINE</span></div>
                        <div>node.uptime {uptime}</div>
                        <div>
                            pretentious.bullshit{" "}
                            <span
                                className="tabular-nums"
                                style={{
                                    color: bullshit >= 100 ? "rgb(248 113 113 / 0.9)" : "rgba(255,255,255,0.7)",
                                }}
                            >
                                {Math.floor(bullshit)}%
                                {bullshit >= 100 && (
                                    <span className="text-red-400/80 ml-1">▲ CRITICAL</span>
                                )}
                            </span>
                        </div>
                    </div>
                    ...
                    <div
                        className="absolute top-6 right-12 font-mono text-[10px] tracking-[0.2em] uppercase leading-relaxed text-right"
                        style={{ color: "rgb(80 140 230 / 0.7)" }}
                    >
                    ...
                    <div
                        className="absolute bottom-8 right-12 font-mono text-[10px] tracking-[0.2em] uppercase flex items-center gap-2"
