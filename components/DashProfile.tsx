...
export default function DashProfile() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (Object.keys(formData).length === 0) { setUpdateUserError("No changes were made"); return; }
        if (imageFileUploading) { setUpdateUserError("Please wait for the image to finish uploading"); return; }
        try {
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                >
                    ...
                    <img
                        ...
                        alt="avatar"
                        className={`w-full h-full object-cover ${imageFileUploadProgress && Number(imageFileUploadProgress) < 100 ? "opacity-50" : ""}`}
                    />
                </div>
